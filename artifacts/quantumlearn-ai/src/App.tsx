...
function QuantumLabView({ learner, setLearner, onGatesChange }: { learner: Learner; setLearner: (value: Learner) => void; onGatesChange: (gates: Gate[]) => void }) {
  ...
  const addGate = (name: GateName) => { setGates((current) => { const next = [...current, { id: Date.now() + current.length, name, qubit: selectedQubit }]; onGatesChange(next); return next; }); setRan(false); setExplanation(''); };
  const run = () => { setLastRun({ gates: gates.map((gate) => ({ ...gate })), probabilities: result.probabilities }); setRan(true); setLearner({ ...learner, runs: learner.runs + 1 }); celebrate({ title: 'Simulation complete!', copy: gates.length ? `${gates.length} gate${gates.length === 1 ? '' : 's'} executed successfully.` : 'The empty circuit has been measured.', tone: 'teal' }); };
...
function Quiz({ learner, setLearner }: { learner: Learner; setLearner: (value: Learner) => void }) {
  ...
  const submit = () => { setSubmitted(true); setLearner({ ...learner, quizScore: Math.round(score / questions.length * 100) }); celebrate({ title: 'Practice complete!', copy: `You scored ${score}/${questions.length}. Keep building the intuition.` }); };
